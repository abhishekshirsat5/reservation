...
from .models import RoomsAvailable, RoomsList, Bookings
...
def loginPage(request):
    if request.user.is_authenticated:
        myorder = Bookings.objects.all().filter(email=request.user.email)
...
def confirmPage(request):
    if request.method == "POST":
        ...
        if mode == "onlinepayment":
            ...
            book = Bookings.objects.create(number=number,
                                           address=addressU,
                                           ...
                                           cin=cin,
                                           cout=cout,
                                           amount=int(request.POST['amounts']),
                                           mode=mode,
...
def cancelOrder(request):
    if request.method == "POST":
        ...
        Bookings.objects.get(id=ids).delete()
